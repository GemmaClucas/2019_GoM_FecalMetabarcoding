...
This is what I found was deep enough in the 2017 and 2018 tern samples, and so I'll use it here too.
...
Here I can use both Jaccard distances (presence/absence only) and Bray-Curtis distances (takes into account RRA). I will just use the 400 depth data.
...
```
The emperor plot doesn't show any differentiation between the robot and spin-column extractions, but the p-value for the PERMANOVA is significant (pseudo-F = 3.713, p < 0.001). I'm not sure how meaningful this is though, if the PCA doesn't show any clustering
...
```
Here, the emperor plot also doesn't show any clustering, and the PERMANOVA is not significant (pseudo-F = 0.847, p = 0.539). 