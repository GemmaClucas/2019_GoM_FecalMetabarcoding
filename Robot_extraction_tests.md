...
and so I’ll use it here too. I am also testing 1000 reads to see if it
makes a difference to anything, as it might pick up more low abundance
taxa.
...
A few of the samples have dropped out from one plate or the other, so to
be able to fairly compare them, I need to exclude samples that do not
have a pair on the other plate.
...
I have deleted these from the `samples-to-keep.txt` file. To remove
them, run:
...
No difference in diversity among groups whether rarefying to a depth of
1000 (H = 1.729, p = 0.185) or 400 (H = 2.00, p = 0.157).

After matching the samples, there is also not a significant difference
between extraction methods (H = 2.93, p = 0.0865)

## 9. Beta diversity to test for difference in composition among groups

Here I can use both Jaccard distances (presence/absence only) and
Bray-Curtis distances (takes into account RRA). I will just use the 400
depth data but do it for the matched samples.
...
The emperor plot doesn’t show any differentiation between the robot and
spin-column extractions, but the p-value for the PERMANOVA is
significant (pseudo-F = 3.207, p \< 0.001). I’m not sure how meaningful
this is though, if the PCA doesn’t show any clustering
...
Here, the emperor plot also doesn’t show any clustering, and the
PERMANOVA is not significant (pseudo-F = 0.399, p = 0.94).
